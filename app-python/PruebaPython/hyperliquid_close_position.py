#!/usr/bin/env python3
"""
Script para cerrar posiciones a market price en Hyperliquid
"""
...
def display_positions(positions):
    """Mostrar posiciones disponibles para cerrar"""
    if len(positions) == 0:
        print("✅ No hay posiciones abiertas para cerrar")
        return False
    
    print("\n" + "="*60)
    print("📈 POSICIONES DISPONIBLES PARA CERRAR")
    print("="*60)
...
def close_position(exchange, coin, size):
    """Cerrar una posición específica a market price"""
    try:
        print(f"\n🚀 Cerrando posición {coin} de tamaño {abs(size):.6f} a market price...")
        
        # Para cerrar una posición, necesitamos hacer lo opuesto
        # Si es SHORT (size < 0), necesitamos COMPRAR (is_buy = True)
        # Si es LONG (size > 0), necesitamos VENDER (is_buy = False)
        is_buy = size < 0  # Si size es negativo (SHORT), compramos para cerrar
        
        print(f"📋 Ejecutando {'COMPRA' if is_buy else 'VENTA'} de {abs(size):.6f} {coin} a market price...")
        ...
        show_payload(coin, is_buy, abs(size), reduce_only=False)
        
        # Usar market_open para cerrar la posición
        order_result = exchange.market_open(coin, is_buy, abs(size), None, 0.01)
...
def main():
    """Función principal"""
    print("🚀 Iniciando cierre de posiciones en Hyperliquid...")
    ...
    # Preguntar al usuario qué posición cerrar
    print(f"\n🔍 Selecciona la posición que quieres cerrar (1-{len(positions)}):")
    ...
    try:
        ...
        size = float(selected_position.get("szi", 0))
        
        # Confirmar la acción
        side = "LONG" if size > 0 else "SHORT"
        print(f"\n⚠️  ¿Estás seguro de que quieres cerrar tu posición {coin} {side}?")
        print(f"   📊 Tamaño: {abs(size):.6f} {coin}")
        print(f"   📈 PnL actual: ${float(selected_position.get('unrealizedPnl', 0)):,.2f}")
        
        confirm = input("\nEscribe 'SI' para confirmar: ")
        
        if confirm.upper() == "SI":
            close_position(exchange, coin, size)
            
            # Esperar un momento y mostrar el estado actualizado
            print("\n⏳ Esperando 3 segundos para verificar el cierre...")
            time.sleep(3)
            
            # Verificar el estado actualizado
            updated_positions = get_open_positions(info, address)
            if len(updated_positions) == 0:
                print("✅ ¡Posición cerrada exitosamente! No hay posiciones abiertas.")
            else:
                print(f"📊 Estado actualizado - {len(updated_positions)} posiciones restantes")
